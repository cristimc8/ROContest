...
<!DOCTYPE HTML>
<head>
  <title>ROContest - <?php echo $_SESSION['username']; ?></title>
...
<ul>
  ...
  <form action="copertaprofil.php" method="POST" enctype="multipart/form-data" id = "content">
  ...
  <form method="POST" action="picprofil.php" enctype="multipart/form-data" id = "content">
      ...
      <p style = "position: relative; top: -65px; left: 0; right: 0; margin: auto; font-size: 18px;"><?php echo $usr; ?></p>
      <input type="submit" name="submit" value="Salveaza poza" id = "classic" style=" border-radius: 0;">
    </div>
  </form>

  <div class="freespace">
  </div><div style = "padding-bottom: 280px;"></div>
 <div class="row">
  ...
  <div class="card-panel col s12 m12 l6 offset-l3" style = "background-color: rgb(17, 32, 56); text-align: center;">
    ...
    $picString = "ViewImage.php?id=";
    for($i = $lastID; $i >= 1; $i--){
      $sql = "SELECT tip, op, concurs, descriere FROM `pictures` WHERE id = '$i' AND op = '$usr'";
      $result = mysqli_query($conn, $sql);
